...
parser.add_argument('--data_class', type=int, default=2,
                    help = '1 for Dermofit, 2 for Dermatomyositis TilingOnly, 3 for Dermatomyositis interpolateOnly')
parser.add_argument('--exp', type=str,
                    default='Dermatomyositis_prototype/', help='experiment_name')
...
def train_self_supervised(args, snapshot_path):
    ...
    logging.info(f"Total silices is: {total_slices}")
    ...
    for epoch_num in iterator:
        for i_batch, sampled_batch in enumerate(trainloader):
            ...
            model_ssim_loss=ssim_loss(outputs1,outputs2) 
            loss = 1 - model_ssim_loss 
            ...
            writer.add_scalar('loss',
                              loss.detach().item(), iter_num)
            logging.info(f"iteration {iter_num} : model loss :{loss.item()}")
...
if __name__ == "__main__":
    ...
    shutil.copytree('.', snapshot_path + '/code',
                    shutil.ignore_patterns(['.git', '__pycache__', '.out', 'pytorch_ssim']))

    logging.basicConfig(filename=snapshot_path+"/log.txt", level=logging.INFO,
                        format='[%(asctime)s.%(msecs)03d] %(message)s', datefmt='%H:%M:%S')
    logging.getLogger().addHandler(logging.StreamHandler(sys.stdout))
    logging.info(str(args))
    train_self_supervised(args, snapshot_path)
